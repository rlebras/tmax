...
#
# Pinned to commit 9ec2e76a on self_test_v3. No extra flags are needed to
# enable the gate; this is otherwise identical to
# launch_tmax9b_tb21_ctxmgmt.sh (context management, no gate) and
# launch_tmax9b_tb21_replicate.sh (pre-context-management baseline), so all
# three runs are directly comparable. See docs/vanillux2_self_test_gate.md
# for the mechanism, and its "Measurement" section for what this run is
# meant to produce: the finished-wrong / step-exhaustion / stopped-early
# outcome taxonomy, submitted_with_failing_checks rate, and self-test
# adoption / added steps-tokens vs. the baselines — computed per arm with
